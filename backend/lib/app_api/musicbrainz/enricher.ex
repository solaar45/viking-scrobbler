defmodule AppApi.MusicBrainz.Enricher do
  ...
  defp enrich_from_recording(listen) do
    case Client.fetch_recording(listen.recording_mbid) do
      ...
      {:error, reason} ->
        Logger.warning(
          "MusicBrainz fetch failed for #{listen.recording_mbid}: #{inspect(reason)}"
        )
  ...
  defp update_listen_metadata(listen, mb_data) do
    ...
    # Merge with new data
    updated_metadata =
      Map.merge(existing_metadata, mb_data, fn
        # Prefer new data if not nil
        _key, existing, new -> new || existing
      end)
