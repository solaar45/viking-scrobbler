defmodule AppApi.MusicBrainz.Client do
  ...
  """
  def search_recording(artist_name, track_name) when is_binary(artist_name) and is_binary(track_name) do
      ...
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} ->
        case Jason.decode(body) do
          {:ok, %{"recordings" => [first | _]}} ->
            ...
            if score >= 85 do
              Logger.info("Found MBID for '#{track_clean}' by #{artist_clean}: #{mbid} (score: #{score})")
              {:ok, mbid}
            else
